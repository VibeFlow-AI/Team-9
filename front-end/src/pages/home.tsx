...
export default function HomePage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { data, error: authError } = await authClient.signIn.social({
        provider: "google",
        callbackURL: "/role-selection", // Redirect after successful auth
      });
      ...
      // The redirect and success will be handled by the OAuth flow
    } catch (err) {
      setError("An unexpected error occurred. Please try again.");
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      if (authError) {
        setError(authError.message || "GitHub login failed. Please try again.");
        setIsLoading(false);
        return;
      }
